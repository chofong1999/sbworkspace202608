...
本章接續DOM與Event，使用老師`week6\day3\DOM_FormCheck.html`示範：在表單真正送出以前讀取欄位、檢查資料，並在驗證失敗時取消送出。
...
建立`C:\jscode\day3\DOM_FormCheck.html`：
...
這份整理版把`<script>`放在`</body>`之前，因此表單元素建立完成後才執行查找。老師原檔把`<script>`寫在`</body>`之後，多數瀏覽器仍可容錯執行，但不是建議的HTML結構。
...
老師範例把JavaScript寫入`action`，目的是讓課堂上不用後端也能看見「表單確實繼續送出」的結果。正式網站應把`action`設為伺服器端URL，例如：
...
老師原檔只在空欄位分支呼叫`event.preventDefault()`；`C:\jscode\day3\DOM_FormCheck.html`目前的個人修改版則在函式開頭無條件呼叫，並註解掉分支內的呼叫。
