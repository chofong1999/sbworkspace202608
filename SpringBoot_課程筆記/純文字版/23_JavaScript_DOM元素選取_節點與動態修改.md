...
本章以老師的`week6\day2`、`day3`原檔為主，整理DOM元素選取、內容、節點、表格、Attribute與Event。`C:\jscode`作為可執行工作副本；若其中有個人延伸，會明確標示。
...
建立`C:\jscode\day3\DOM_ChangeCSS.html`：
...
本段是安全性補充；課堂原始碼目前沒有加入此判斷。

## 9. 執行與驗證

1. 在VS Code開啟`C:\jscode`資料夾。
...
老師檔案：`week6\day2\DOM_basic.html`
...
`forEach`的callback也可以接收第二個`index`參數；`C:\jscode`個人練習版利用它一併印出索引。
...
課堂程式印出`link.href`。這個DOM property通常會回傳瀏覽器解析後的絕對網址；如果要讀取HTML原本寫下的相對值，可使用：
...
老師檔案：`week6\day2\DOM_Node2.html`
...
`C:\jscode`個人練習版另加入全域`num`及顏色Array，讓多次點擊時編號持續增加，顏色以`num % color.length`循環。這是Array與餘數運算的延伸，不是老師原檔內容。
...
老師檔案：`week6\day2\DOM_Table.html`
...
老師檔案：`week6\day3\DOM_attribute.html`
...
老師檔案：`week6\day3\DOM_NodeRemove.html`

課堂版本從`firstChild`開始處理，並特別判斷`#text`，原因是HTML縮排可能使第一個Node成為空白Text Node。較直接且穩定的Element寫法是：
...
索引`0`代表第一列。課堂版本直接執行`deleteRow(0)`；若表格已經沒有列，再執行會發生索引錯誤，因此可先檢查`rows.length`。

## 16. DOM Event與事件物件

老師檔案：`week6\day3\DOM_Event.html`
...
課堂HTML另以`onmouseout`把背景改回灰色。這可觀察滑鼠進入與離開元素時的兩個事件。
...
老師範例透過`document.onkeydown`取得Event並判斷`Ctrl + Y`或`Ctrl + Z`。原檔使用`keyCode`與`which`，這兩個舊Property已不建議在新程式中使用；現代寫法可使用`event.key`：
