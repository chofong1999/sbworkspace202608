...
目前已核對`C:\jscode\day1\`的四個原始檔：`first.html`直接載入`basis.css`；`html_js.html`直接在`<script>`內寫JavaScript；`first.js`是內容相近的獨立JavaScript檔，可由Node.js執行。兩份HTML都沒有`<script src="first.js">`，所以`first.js`目前不是由瀏覽器頁面載入。
...
```text
C:\jscode\day1\selectors.html
C:\jscode\day1\images\img28.jpg
```
...
本章保留課堂使用的`var`寫法。現代JavaScript也常用`let`限制變數作用域：
...
```powershell
PS C:\jscode> cd day1
PS C:\jscode\day1> node first.js
55
```
...
這時瀏覽器會以`first.html`為基準尋找同層的`first.js`。但這是外部檔案的另一種組織方式；目前課堂的`html_js.html`仍使用內嵌`<script>`，不可把建議寫法誤當成目前檔案已完成的狀態。
...
| `node first.js`找不到檔案 | Terminal不在`C:\jscode\day1` | 先執行`cd C:\jscode\day1`，再執行Node命令 |
