...
本章以老師的`week6\day2`原檔整理BOM。BOM（Browser Object Model）是瀏覽器提供給JavaScript操作視窗、網址、定時器等瀏覽器環境的API；`C:\jscode\day2`目前有少量個人調整，差異會另外標示。
...
老師原檔停止Timer後會保留畫面上的最後時間。若希望介面清楚顯示狀態，可自行改成：
...
`C:\jscode`個人練習版目前另把`intervalId`寫入頁面，因此按Stop後看到數字是Timer ID，不是老師原檔的結果。

## 6. 定時切換圖片

課堂檔案：`bom_images.html`
...
```text
C:\jscode\day2\bom_images.html
C:\jscode\day2\images\banana.png
C:\jscode\day2\images\grape.png
...
```

## 7. 防止重複啟動多個Timer

目前課堂範例每按一次Start就建立一個新的Interval，並只把最新ID存入`intervalId`。若連按多次：
...
1. 對`C:\jscode\day2\bom_window.html`選擇`Open with Live Server`。
...
4. 老師原檔會保留停止當下的最後時間；若本機練習版顯示數字，該數字是Timer ID。

### 8.3 圖片輪播

1. 開啟`bom_images.html`。
2. 確認初始圖片為banana。
3. 按`Start Image`，確認老師原檔約每3秒切換一次；`C:\jscode`個人版目前改為約每秒一次。
...
| 個人練習版Stop後顯示數字 | 個人版把Timer ID寫入DOM | 若要狀態文字，改寫成`已停止`；老師原檔沒有這一行 |
