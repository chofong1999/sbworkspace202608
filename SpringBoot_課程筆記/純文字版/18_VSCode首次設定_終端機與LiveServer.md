...
本章記錄的`CET fatal error`與PowerShell選擇，是特定教室電腦上已發生的狀況。其他電腦若原本的PowerShell可正常使用，不需要為了照抄範例而強制更換。
...
| 顯示名稱 | 執行檔路徑 | 本次結果 |
|---|---|---|
| `Windows PowerShell` | `C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe` | 可正常執行，選為預設 |
| `PowerShell` | `C:\Users\student\AppData\Local\Microsoft\WindowsApps\...` | 在本機出現CET fatal error，不選 |

因此本次選擇：

`Windows PowerShell`

選擇的依據不是名稱較舊或較新，而是該實際路徑在目前電腦上能否正常啟動。
...
| 終端機啟動出現CET fatal error | 實際PowerShell路徑 | 改選本機可正常啟動的Shell；本次為Windows PowerShell 5.1路徑 |
