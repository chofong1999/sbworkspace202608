...
```text
前端：day5/jquery_employee.html、day5/js/api.js
後端：sbemployeetest
```

> 語法速查：[Spring Data JPA](../語法字典/05_Spring_Data查詢交易與分頁.md)｜[jQuery AJAX](../語法字典/13_jQuery與AJAX.md)
...
範例先以`equals.html`比較字串`"10"`與數字`10`：
...
範例專案使用Spring Boot 3.3.2與Java 17。JDK 21可以執行以Java 17為目標版本的專案。
...
`create-drop`會在啟動時建立資料表、停止時刪除資料表，適合練習環境，不適合保存正式資料。
...
上例沿用範例中的手動編號方式。若資料表可能為空，`max(...).orElseThrow()`會失敗；一般專案宜讓資料庫自動編號，或另外定義空表時的初始值。

## 7. 建立前端HTML

在`day5/`建立`jquery_employee.html`：
...
- `@CrossOrigin("*")`適合開發測試，正式環境應限制來源。
- `api.js`可以使用不同排版或callback寫法，但完整版本必須涵蓋查詢、新增、修改、刪除及錯誤處理流程。
