...
本章依序整理`const`、資料型別、Truthiness、Dialog、輸入轉型、條件判斷、迴圈、函式與外部Script；每一節都附有可直接執行的最小範例。

> 語法速查：[JavaScript核心](../語法字典/11_JavaScript核心_陣列與物件.md)

## 本章快速索引

- [1. `const`限制的是重新指派](#1-const限制的是重新指派)
- [2. `typeof`與JavaScript型別](#2-typeof與javascript型別)
- [3. `prompt`回傳String](#3-prompt回傳string)
- [4. Truthy與Falsy](#4-truthy與falsy)
- [5. 外部Script與函式回傳值](#5-外部script與函式回傳值)
- [6. 產生指定範圍的亂數整數](#6-產生指定範圍的亂數整數)
- [7. `while`猜數字與巢狀三元運算](#7-while猜數字與巢狀三元運算)
- [8. 比較三個亂數的最大值](#8-比較三個亂數的最大值)
- [9. 執行方式](#9-執行方式)
- [10. 常見錯誤](#10-常見錯誤)
- [11. 本章檢查表](#11-本章檢查表)
...
`return value`把計算結果交回呼叫處，因此`result`可以取得總和。若函式沒有明確`return`，呼叫結果通常是`undefined`。直接把`prompt`的String交給`sum`時，`i <= n`會發生數值轉型；正式輸入處理仍建議先使用`Number(...)`並驗證結果，避免依賴隱式轉型。
...
範例程式把第二個三元運算放在false分支，形成「太小／太大／答對」三種結果。雖然可執行，但巢狀三元運算可讀性較低；流程較長時，`if / else if / else`通常更清楚。
