...
本章接續第26章的前後端分離，練習以`$.ajax()`讀取專案內JSON、外部公開API及自己的Spring Boot API，最後依HTTP狀態碼處理錯誤。前端範例放在`day4/`，後端範例專案名稱為`sbstatus0821`。

> 語法速查：[MVC與REST](../語法字典/02_Spring_MVC與REST.md)｜[jQuery狀態處理](../語法字典/13_jQuery與AJAX.md#ajax-status)
...
也可以在欄位上使用`@Autowired`注入DAO；上例採用等效的建構子注入，使必要依賴不可省略，也較容易測試。兩者都能運作：
...
1. 先以Spring Boot App啟動`sbstatus0821`後端專案。
2. 確認Console顯示應用程式已在8080啟動。
3. 在VS Code開啟包含`day4`的前端練習資料夾。
...
- [ ] 能說明空商品List為何回204，以及用`Math.max()`計算ID時應如何處理空List。
