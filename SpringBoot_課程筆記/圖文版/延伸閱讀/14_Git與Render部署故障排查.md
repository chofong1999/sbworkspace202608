...
*圖1：遠端main已有本機沒有的提交，因此Push被拒絕並提示fetch first；應先整合遠端歷史，不能把Force Push當成一般修正。*
...
*圖2：相同Products API在localhost成功、Render只回Not Found；這個對照能先確認原始碼端點存在，再繼續判斷部署、路由或服務啟動層。*
