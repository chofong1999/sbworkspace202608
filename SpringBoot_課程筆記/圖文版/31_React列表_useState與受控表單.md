...
> 語法速查：[JavaScript陣列](../語法字典/11_JavaScript核心_陣列與物件.md#array-methods)｜[React State與表單](../語法字典/14_React_Vite_JSX與Hooks.md#usestate)
...
TodoList會把Array CRUD與本章State更新方式組合起來；LoginForm會加入表單提交；`useEffect`、`fetch()`及API載入狀態集中整理於[第32章](32_React_TodoList_LoginForm與useEffect_API資料擷取.md)。
